...
module.exports = async args => {
  if (!args.message.guild || !args.message.guild.id) {
    return await replyOrSend(`${ERROR_PREFIX}You can only change the prefix for a Discord Guild.`,
      args.message)
  }
  ...
  if(suggestedPrefix.match(/\s/g)) {
    return await replyOrSend(`${ERROR_PREFIX}The prefix can't have any white-space symbols in it.`,
      args.message)
  }
