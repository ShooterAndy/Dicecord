...
module.exports = async args => {
  if(!args.commandText) {
    return await replyOrSend(nws`${ERROR_PREFIX}No choices to pick from. Please input something like this:\n\
  ...
  if(choiceParts.length < MIN_PICK_NUMBER) {
    return await replyOrSend(nws`${ERROR_PREFIX}Not enough choices to pick from, please provide at least \
      ${MIN_PICK_NUMBER}.`, args.message)
  }

  if(choiceParts.length > MAX_PICK_NUMBER) {
    return await replyOrSend(nws`${ERROR_PREFIX}Too many choices to pick from, please provide fewer than \
  ...
  if(choices.length < MIN_PICK_NUMBER) {
    return await replyOrSend(nws`${ERROR_PREFIX}Not enough actual choices to pick from, please provide at \
      least ${MIN_PICK_NUMBER}.`, args.message)
  }

  return await replyOrSend(choices[random.integer(0, choices.length - 1)], args.message)
}