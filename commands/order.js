...
module.exports = args => {
  if(!args.commandText) {
    return reply(nws`${ERROR_PREFIX}No items to order. Please input something like this:\n\
  ...
  if(choiceParts.length < MIN_ORDER_NUMBER) {
    return reply(nws`${ERROR_PREFIX}Not enough items to order, please provide at least \
      ${MIN_ORDER_NUMBER}.`, args.message)
  }
  if(choiceParts.length > MAX_ORDER_NUMBER) {
    return reply(nws`${ERROR_PREFIX}Too many items to order, please provide less than \
  ...
  if(choices.length < MIN_ORDER_NUMBER) {
    return reply(nws`${ERROR_PREFIX}Not enough actual items to order, please provide at least \
      ${MIN_ORDER_NUMBER}.`, args.message)
  }
  ...
  return reply('Your list of items, randomly ordered:\n```' + choicesString + '```',
    args.message)
}