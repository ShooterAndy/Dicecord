...
module.exports = async args => {
  if (!args.message.guild || !args.message.guild.id) {
    return replyOrSend(`${ERROR_PREFIX}You can only change the prefix for a Discord Guild.`,
      args.message)
  }

  if(!args.message.member || !args.message.member.permissions.has('ADMINISTRATOR')) {
    return replyOrSend(nws`${ERROR_PREFIX}Only a guild administrator can change the prefix for this \
      Discord Guild.`, args.message)
  }

  if(!args.commandText || args.commandText.trim().length === 0) {
    return replyOrSend(`${ERROR_PREFIX}No prefix specified. Please input something like this:\n\
  ...
  if(suggestedPrefix.length < MIN_PREFIX_LENGTH) {
    return replyOrSend(nws`${ERROR_PREFIX}Please provide a prefix that is longer than \
      ${MIN_PREFIX_LENGTH} symbols.`, args.message)
  }
  if(suggestedPrefix.length > MAX_PREFIX_LENGTH) {
    return replyOrSend(nws`${ERROR_PREFIX}Please provide a prefix that is shorter than \
      ${MAX_PREFIX_LENGTH} symbols.`, args.message)
  }

  if(suggestedPrefix.match(/\s/g)) {
    return replyOrSend(`${ERROR_PREFIX}The prefix can't have any white-space symbols in it.`,
      args.message)
  }

  if(suggestedPrefix.startsWith('/')) {
    return replyOrSend(nws`${ERROR_PREFIX}the prefix can't start with the Discord-reserved symbol \`/\`.`,
      args.message)
  }
  ...
  if(suggestedPrefix.indexOf('#') !== -1 ||
    ...
    suggestedPrefix.indexOf('"') !== -1 ||
    suggestedPrefix.indexOf('||') !== -1) {
    return replyOrSend(nws`${ERROR_PREFIX}The prefix can't have either double or single quotation marks, \
