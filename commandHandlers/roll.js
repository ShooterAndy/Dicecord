...
let rollNameSpace = function () {
  ...
  const processMessage =  async (_interaction, args) => {
    warnings = []
    throws = []
  ...
  const processRollFormula = thisThrow => {
    ...
    if (disallowedSymbols && disallowedSymbols.length) {
      addWarning(nws`there were unrecognized symbols such as \`${disallowedSymbols[0]}\` found \
          in \`${thisThrow.originalFormula}\`, so they will be ignored. Perhaps you forgot to \ 
          place a \`${COMMENT_SEPARATOR}\` symbol before writing a comment`)
    ...
    const tryToProcessOperand = () => {
      ...
      if (unrecognizedSymbols && unrecognizedSymbols.length) {
        addWarning(nws`there were unrecognized symbols such as \
              \`${unrecognizedSymbols[0]}\` found in \`${thisThrow.originalFormula}\`, so they \ 
              will be ignored`)
  ...
  return {
    ...
    goOnFromWarning: goOnFromWarning,
    repeatRollCommand: repeatRollCommand
  }
}();

module.exports = async (interaction, args) => {
  await rollNameSpace.processMessage(interaction, args)
}
